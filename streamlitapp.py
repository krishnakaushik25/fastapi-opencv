...
def drawboundingbox(img, boxes,pred_cls, rect_th=2, text_size=1, text_th=2):
    ...
    class_color_dict = {
        cat: [random.randint(0, 255) for _ in range(3)] for cat in pred_cls
    }
...
if bytesObj or url:
    ...
    response = requests.put("http://52.34.237.224/predict",data = payload,headers={"Content-Type": "application/json; charset=utf-8"})
